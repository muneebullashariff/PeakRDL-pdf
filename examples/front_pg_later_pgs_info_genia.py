...
############################################################################
genia_logo = "genia_logo.png"

def myFirstPage(canvas, doc):
    canvas.saveState()

    # Genia Logo
    canvas.drawImage(genia_logo,405,720,width=140,height=60,preserveAspectRatio=True,mask='auto') 

    # Genia FPGA
    text_color = colors.HexColor(0x18325e)
    canvas.setFillColor(text_color)
    canvas.setFont('Helvetica-Bold', 22)
    canvas.drawString(410, 620, 'Genia ')
    ...
    # 8 Million Platform
    text_color = colors.HexColor(0x4d82aa)
    canvas.setFillColor(text_color)
    canvas.setFont('Helvetica-BoldOblique', 12)
    canvas.drawString(429, 580, '8 Million Platform')

    # FPGA B0 Register Specification 
    canvas.setFillColor(doc_color)
    canvas.setFont('Times-Bold', 25)
    canvas.drawString(179, 500, 'FPGA B0 Registers Specification')
    ...
    canvas.drawString(270, 110, 'Genia Corporation')
    ...
    canvas.setFillColor(doc_color)
    canvas.setFont('Times-Roman', 8)
    strg = "Copyright \xa9 " + datetime.date.today().strftime('%Y') + "  - Genia Corporation, All Rights Reserved"
...
def myLaterPages(canvas, doc):
    ...
    # Header Genia logo
    canvas.drawImage(genia_logo,500,790,width=70,height=30,preserveAspectRatio=True,mask='auto') 
